...
//		val maxsuffix = suffixes(0)
		...
		val validDatum = datum.containsPredicates
		...
		println("@gpreds\t0 %s".format(datum.predicateIndices.mkString(" "))) 
		...
		if (connect && validDatum) 
			extractConnectionFeatures(datum, labelConnect, datum.predicateIndices.toArray, maxdist)
		println
	}
...
//			println("pred(%d)\t+pred-%S".format(i, datum.words(i-1)))
		...
		if (argfeats) {
			val argis = datum.args
			for (i <- 1 to slen) {
				val isArg = if (argis.contains(i) && labelCorrect) "+" else ""
				println("isArg(%d)\t%s%s".format(i, isArg, SRLFeatures.argFeatures(i, tokens).mkString(" ")))															
			}
		}
	}
