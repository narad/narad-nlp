...
trait BigramTaggerFeatures extends TaggerFeatures {
    ...
    val reader = new CoNLLReader(in)
		reader.zipWithIndex.foreach { case(datum, i) =>
          ...
          val isCorrect1 = tag == datum.getAttribute(widx+1, params.MODE) //correct(datum, widx+1, params.MODE) //datum.postag(widx+1) == tag
          out.write("ulabel(%d,%s)\t%s%s\n".format(widx+1, tag, if (isCorrect1) "+" else "", builder.toString().trim))
           ...
           //println("word = " + word + "; prevword = " + prevword)
						val bigramfeats = bigramFeatures(datum, widx, widx+1, params)
						...
						tags.zipWithIndex.foreach { case(btag, btidx) => 
							...
							val isCorrect2 = datum.getAttribute(widx+1, params.MODE) == tag &&
							                 datum.getAttribute(widx, params.MODE) == btag //correct(datum, widx+1, params.MODE) == tag && correct(datum, widx, params.MODE) == btag //datum.postag(widx+1) == tag && datum.postag(widx) == btag
