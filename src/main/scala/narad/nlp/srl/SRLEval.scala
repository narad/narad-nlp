...
object SRLEval {
  ...
  def constructFromBeliefs(beliefs: Array[Potential], words: Array[String], tags: Array[String],
    ...
    val args = Array.ofDim[String](slen, slen)

    for (i <- 0 until slen) preds(i) = "_"
    for (i <- 0 until slen; j <- 0 until slen) args(i)(j) = "_"
...
/*
    ...
    beliefs.filter(b => b.name.contains("sense(")).foreach { p =>
      val SENSE_PATTERN(pidx) = p.name
    ...
    for (belief <- abeliefs) {
      val ARG_PATTERN(pred, dep) = belief.name
...
/*
    val argGroups = beliefs.groupBy { b =>
      b.name match {
        case ARG_PATTERN(pidx, aidx) => (pidx.toInt, aidx.toInt)
        case _=> (-1, -1)
      }
    }
    val labelGroups = beliefs.groupBy { b =>
      b.name match {
        case ARG_PATTERN(pidx, aidx) => (pidx.toInt, aidx.toInt)
        case _=> (-1, -1)
      }
    ...
    case LABEL_PATTERN(pidx, aidx, label) => (pidx.toInt, aidx.toInt)
